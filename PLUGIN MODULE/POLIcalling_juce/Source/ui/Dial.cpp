    ...
    , paramAttachment (audioParam, [&] (float v) { updateValue (v); }, um)
{
    ...
    label.setText (audioParam.getName (12), juce::NotificationType::dontSendNotification);
...
void Dial::paint (juce::Graphics& g)
{
    ...
    if (knobImage.isValid())
    {
        ...
        float angle = rotaryStartAngle + value * (rotaryEndAngle - rotaryStartAngle);

        juce::AffineTransform transform = juce::AffineTransform::rotation(angle, knobImage.getWidth() / 2.0f, knobImage.getHeight() / 2.0f)
            .translated(center.x - knobImage.getWidth() / 2.0f,
                center.y - knobImage.getHeight() / 2.0f);
    ...
    else
    {
        ...
        g.drawFittedText("Image missing", getLocalBounds(), juce::Justification::centred, 1);
    }
}
